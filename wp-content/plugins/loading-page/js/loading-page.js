...
(function(){
	...
	{
		var time = ('additionalSeconds' in lp.options && jQuery.isNumeric( lp.options[ 'additionalSeconds' ] ) ) ? parseInt(lp.options[ 'additionalSeconds' ]) : 0,
			complete = function(){
	...
	if(lp.options['loadingScreen'] && lp.validateScreenSize())
	{
		if(lp.options['closeBtn'])
		{
			...
			close_btn.click(lp.destroyLoader);
